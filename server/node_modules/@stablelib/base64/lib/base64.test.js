...
describe("Base64", () => {
    it("should correctly encode test vectors", () => {
        testVectors.forEach((v) => {
            const input = new Uint8Array(v[0]);
            expect(encode(input)).toBe(v[1]);
        });
    });
    it("should correctly decode test vectors", () => {
        testVectors.forEach((v) => {
            const output = new Uint8Array(v[0]);
            expect(decode(v[1]).toString()).toBe(output.toString());
        });
    });
...
describe("Base64 URL-safe", () => {
    ...
    it("should correctly encode test vectors", () => {
        testVectors.forEach((v) => {
            const input = new Uint8Array(v[0]);
            expect(encodeURLSafe(input)).toBe(urlSafe(v[1]));
        });
    });
    it("should correctly decode test vectors", () => {
        testVectors.forEach((v) => {
            const output = new Uint8Array(v[0]);
            expect(decodeURLSafe(urlSafe(v[1])).toString()).toBe(output.toString());
        });
    });
    it("should throw when decoding incorrect strings", () => {
        badVectors.forEach((v) => {
